...
class Book {
	constructor(title, author, pageLen, isRead, coverImg, description, id) {
...
function displayBooks(
	...
	if (!document.querySelector(".book.book-add")) {
		bookCanvas.insertAdjacentHTML(
			"beforeend",
			`<div class="newButtonContainer popIn">
...
function addEvents() {
	...
	const descriptions = Array.from(
		document.querySelectorAll(".book-description")
	);
...
function formSubmit(e) {
	...
	const inputArr = [
		...
		document.getElementById("page-len"),
		document.getElementById("img-url"),
	];
	...
	const validArr = [];
	const invalidArr = inputArr.filter((el) => {
	...
	validArr.forEach((el) => {
		const errElem = el.nextElementSibling;
		errElem.textContent = "";
		errElem.className = "error";
	});

	invalidArr.forEach(showError);
...
function showError(el) {
	...
	if (el.validity.patternMismatch) {
		err.textContent =
			"Invalid characters found, only letters, numbers, hyphens, underscores and spaces allowed";
	} else {
        err.textContent = "idk man, u dun f'd up"
    }
}
...
function checkTitle() {
	...
	//Check
	const validity = textyRegex.test(title.value)
		? ""
		: "The title must only contain characters from the following: Letters, numbers, hyphens, underscores and spaces.";
...
function checkAuthor() {
	...
	//Check
	const validity = textyRegex.test(author.value)
		? ""
		: "The author must only contain characters from the following: Letters, numbers, hyphens, underscores and spaces.";
...
function checkDesc() {
	...
	//Check
	const validity = textyRegex.test(desc.value)
		? ""
		: "The description must only contain characters from the following: Letters, numbers, hyphens, underscores and spaces.";
...
function checkLen() {
	...
	//Check
	const validity = numRegex.test(length.value)
		? ""
		: "The length must contain only numbers.";
...
function checkURL() {
	...
	//Check
	const validity = urlRegex.test(url.value)
		? ""
		: "This URL is invalid. Try using a different URL.";
	url.setCustomValidity(validity);
}

function genericCheck(el) {
    el.classList = el.checkValidity() ? "valid" : "invalid" 
}
